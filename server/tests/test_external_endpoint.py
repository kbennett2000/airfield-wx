...
def test_external_carries_genuine_model_observation(client: TestClient) -> None:
    # The fused comfort indices moved to the derived block (Cycle 6c); the
    # external block keeps the genuine provider observation it actually fetched.
    _push(
        client,
        wind_speed_ms=6.0,
    ...
    ext = parsed.external
    assert ext is not None
    assert ext.wind_speed_ms == pytest.approx(6.0)
    assert ext.wind_direction_cardinal == "W"
    assert ext.cloud_cover_pct == 50.0
    # Fused comfort indices are no longer fields on the external block.
    assert not hasattr(ext, "beaufort_force")
    assert not hasattr(ext, "wind_chill_c")
    assert not hasattr(ext, "apparent_temperature_c")
    assert not hasattr(ext, "thsw_index_c")
    assert not hasattr(ext, "et0_mm_hour")
